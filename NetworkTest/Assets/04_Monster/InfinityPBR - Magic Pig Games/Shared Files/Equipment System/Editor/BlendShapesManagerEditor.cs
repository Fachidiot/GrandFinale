...
namespace InfinityPBR
{
    ...
    [Serializable]
    public class BlendShapesManagerEditor : InfinityEditor
    {
        ...
        private void ShowGlobalModifier()
        {
            StartVerticalBox();
            StartRow();
            Label($"Global Modifier {symbolInfo}", $"In some versions of the Editor, blend shape values are doubled. This is a Unity bug. To account for this, " +
                                                   $"the Global Modifier value will modify final blend shape values. If you notice shapes are very very extreme, you may " +
                                                   $"choose to set this value to 0.5f, so that at full value, the actual result is clamped.\n\nIf you notice shapes which " +
                                                   $"make things \"smaller\" tend to invert the mesh, that's a clear indication the shape values are being doubled by the " +
                                                   $"bug in the Editor.", 200, true);
            Manager.globalModifier = Float(Manager.globalModifier, 50);
            EndRow();
            EndVerticalBox();
        }
