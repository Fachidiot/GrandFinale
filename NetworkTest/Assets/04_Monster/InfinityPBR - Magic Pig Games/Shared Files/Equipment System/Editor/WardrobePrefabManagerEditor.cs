...
namespace InfinityPBR
{
    ...
    [Serializable]
    public class WardrobePrefabManagerEditor : InfinityEditor
    {
        ...
        {
            ResetObjectFitStrings();
        }
        
        private void ResetObjectFitStrings() => objectFitStrings = EquipmentObjectObjectNames(PrefabAndObjectManager.equipmentObjects);

        public override void OnInspectorGUI()
        {
            InitialCheck();
            LinkToDocs("https://infinitypbr.gitbook.io/infinity-pbr/equipment-systems/wardrobe-prefab-manager");
        ...
        private void ShowObjectFit()
        {
            StartRow();
            Label($"Object Fit {symbolInfo}", "Select the \"Object Fit\" string for this object. These can be defined " +
                                              "on individual prefabs which are meant to be rigged to this object. This is optional.", 150);
            string[] array = objectFitStrings.ToArray();
            PrefabAndObjectManager.objectFit = objectFitStrings[Popup(ObjectFitIndex(), array, 250)];
            EndRow();
        }

        private int ObjectFitIndex()
        {
            if (!objectFitStrings.Contains(PrefabAndObjectManager.objectFit))
            {
                PrefabAndObjectManager.objectFit = "";
                return 0;
            }

            return objectFitStrings.IndexOf(PrefabAndObjectManager.objectFit);
        }
