...
namespace InfinityPBR
{
    ...
    public class ColorShifterObject : ScriptableObject
    {
        ...
        public void HideUnnamedColors()
        {
            Debug.Log($"Hiding unnamed colors in {name}");
            foreach (var colorItem in colorSets.SelectMany(colorSet => colorSet.colorShifterItems.Where(colorItem => colorItem.name.StartsWith("Unnamed Color"))))
            {
                colorItem.hidden = true;
            }
        }
    }
