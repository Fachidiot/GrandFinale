...
using UnityEngine;
using System; // 'Action' 이벤트를 사용하기 위해
...
public class PlayerStats : MonoBehaviour
{
    ...
    {
        ResetToBaseStats(); 
        CurrentHealth = CurrentMaxHealth; // 체력 풀로 채움
    }
    ...
    public void TakeDamage(float damage) 
    {
        ...
        Debug.Log($"데미지 {damage} 받음. 현재 체력: {CurrentHealth}, 현재 쉴드: {CurrentShield}"); //
    ...
    // 캬캬 (신규) 2. 체력 보정 함수 캬캬
    /// <summary>
    /// 스탯 적용 후, 현재 체력이 최대 체력보다 높으면 최대 체력으로 맞춥니다.
    /// (예: 아이템으로 최대 체력이 150 -> 100이 되었을 때)
    /// </summary>
    ...
    /// <summary>
    /// (ABIL_001) 고정값으로 스탯을 더합니다. (예: 최대 체력 +50)
    /// </summary>
    public void AddStat(string statName, float value) 
    {
        switch (statName)
        {
            case "MaxHealth":
                ...
                break;
            case "BaseDamage":
                Debug.Log($"BaseDamage 증가: +{value} (적용 필요)"); 
                break;
        }
    }
    ...
    /// <summary>
    /// (ABIL_002) 퍼센트(%)로 스탯을 더합니다. (예: 이동 속도 +10%)
    /// </summary>
    ...
    private IEnumerator ShieldRoutine(float amount, float duration) 
    {
        ...
        Debug.Log($"쉴드 {amount} 획득! (총 {CurrentShield})"); 

        yield return new WaitForSeconds(duration); 

        CurrentShield -= amount; 
        if (CurrentShield < 0) CurrentShield = 0; 
        Debug.Log($"쉴드 {amount} 소멸. (남은 쉴드 {CurrentShield})"); 
    }
}